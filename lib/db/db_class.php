	...
	*/
    ...
    public function query($sql)
    {
        ...
        if(self::$sqlType=='select' || self::$sqlType=='show')
        {
            ...
            {
				//多数据库支持并且读写分离
                if (isset(Web::$app->config['DB']['read']))
                {
					//获取ip地址
					$ip = IClient::getIP();

                    $this->connect(IHash::hash(Web::$app->config['DB']['read'], $ip));
                }
                else
                {
                    $this->connect(Web::$app->config['DB']);
                }
        ...
        else
        {
            ...
            {
				//多数据库支持并且读写分离
                if (isset(Web::$app->config['DB']['write']))
                {
                    $this->connect(Web::$app->config['DB']['write']);
                }
                else
                {
                    $this->connect(Web::$app->config['DB']);

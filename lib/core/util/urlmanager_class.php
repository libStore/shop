...
class IUrl
{
	...
	/**
	 * @brief 将Url从Web支持的一种Url格式转成另一个格式。
	...
	private static function getRouteCache()
	{
		...
		//第一次初始化
		$routeList = isset(Web::$app->config['urlRoute']) ? Web::$app->config['urlRoute'] : array();
		if(empty($routeList))
		{
			self::$urlRoute = false;
			return null;
	...
	public static function creatUrl($url='')
	{
		...
		{
			$url = preg_replace_callback("!(?<=/img/)(.*)(?=\.\w+[/|$])!",create_function(
				'$matches',
		...
		$rewriteRule = isset(Web::$app->config['rewriteRule']) ? Web::$app->config['rewriteRule'] : 'native';
	...
	public static function getRefRoute()
	{
		...
		{
			$localUrlType = Web::$app->config['rewriteRule'] == 'pathinfo' ? self::UrlPathinfo : self::UrlNative;
...
class IServerVars_IIS implements IIServerVars
{
	...
	{
		$re= "";
		if( isset($_SERVER['HTTP_X_REWRITE_URL']) || isset($_SERVER['ORIG_PATH_INFO']) )
		{

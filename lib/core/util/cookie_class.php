...
{
	//cookie前缀
	private static $pre = 'web_';

	//默认cookie密钥
	private static $defaultKey = 'appoil';
	...
	{
		if (isset(Web::$app->config['safePre']))
		{
			return Web::$app->config['safePre'];
		}
		else
		{
			return self::$pre;
	...
	{
		if (isset(Web::$app->config['safeLevel']))
		{
			return Web::$app->config['safeLevel'];
		}
		else
		{
			return self::$level;
	...
	{
		$encryptKey = isset(Web::$app->config['encryptKey']) ? Web::$app->config['encryptKey'] : self::$defaultKey;

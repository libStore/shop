...
class IException extends Exception
{
	...
	{
		$web_path = WEB_PATH;
		$iapp_path = Web::$app->getBasePath();

		$count = 1;

		if (strpos($path, $web_path) === 0)
		{
			$path = str_replace(WEB_PATH, "WEB_PATH/", $path, $count);
		}
...
class IHttpException extends IException
{
	...
	public function show()
	{
		...
		//检查用户是否定义了error处理类
		$config = isset(Web::$app->config['exception_handler']) ? Web::$app->config['exception_handler'] : 'Error';
		$flag = class_exists($config);
		if( $flag && method_exists($config,"error{$httpNum}") )
		{
			$errorObj = new $config(Web::$app, 'error');
			call_user_func(array($errorObj,'error'.$httpNum),$errorData);
		}
		//是系统内置的错误机制
		else if (file_exists(WEB_PATH . 'web/view/' . 'error' . $httpNum . $controller->extend))
		{
			$controller->render(WEB_PATH . 'web/view/' . 'error' . $httpNum, $showData);
		}
		//输出错误信息
		else
		{
